// Author : https://github.com/benjiec/scatter-matrix/blob/master/scatter-matrix.js
/* 'ScatterMatrix'라는 객체(클래스 모양 모방)를 생성
 * => 아래에서 인스턴스의 인자값이 주어지지 않을 시에, 초기 속성값을 정해준다. */
ScatterMatrix = function (url, data, dom_id, el) {
    // ?
    this.__url = url;

    // Chart 에 표현할 데이터 유무 판단
    if (data === undefined || data === null){
        this.__data = undefined;    // -> '__data'는 다른 속성값과는 다르게, 'data' 인자를 필수로 요하도록 했다.
    } else {
        this.__data = data;   // -> 서버에서 불러온 '.csv' 내의 데이터를 d3 에서 쉽게 사용할 수 있게, 'Dictionary array' 형태로 바꿔준다.
        /* Row 가 150개인 csv 가  위 코드를 거치면,
         *
         * 0: {species: "setosa", sepal length: "5.1" }
         * 1: {species: "setosa", sepal length: "5.5}
         * ...
         * 150: {species: "virginica", sepal length: "10.4"}
         *
         * 위와 같이 Dictionary type object array 로 변환된다!!
         *
         * */
    }
...
/* 'ScatterMatrix'의 클래스 속성값들을 지정해 준다. */
...
ScatterMatrix.prototype.render = function () {
    ...
    // 'd3'가 이용될 영역 할당
    var container = d3.select(self.__dom_id)
        .append("div")
    ...
    // 'svg' 영역 : 차트가 표현될 영역 => Scatter Plot 이 표현될 영역
    ...
    self.onData(function () {
        var data = self.__data;     // -> 차트에 사용할 데이터는 'ScatterMatrix' 객체의 인스턴스가 속성(인자)으로 가진 것을 이용.

        // Divide variables into string and numeric variables : 데이터가 가진 문자형, 숫자형 데이터들을 나눈다.
        var string_variables = [];
        var original_numeric_variables = [];
        // 인스턴스에 다음 새로운 속성 2가지를 추가한다.
        self.__string_variable_values = {};     // -> 데이터로 부터 파싱한 문자형 데이터 모을 dictionary
        self.__numeric_variables = [];      // -> 데이터로 부터 파싱한 숫자형 데이터를 모을 array

        //console.log(data[0]);

        for (k in data[0]){     // data[0] 는 {species: "setosa", sepal length: "5.1", ..., sepal width: "3.5"}를 뜻 한다.
                                // 'k'는 dictionary 의 'key'를 뜻한다.
            var is_numeric = true;      // 'boolean' 값을 선언 (JS 에서는 가능하네...)

            // 읽어들인 데이터가 숫자형인지, 문자형인지 판별한다.
            data.forEach(function (d) {
                var v = d[k];
                if (isNaN(+v)) is_numeric = false;  // 'isNaN(x)'은 x가 숫자(or 숫자만으로 구성된 문자열)면 false, 문자열이면(or 숫자+특스문자) true 반환!!!
            });
            ...
            if (is_numeric){
                ...
                // string type (딕셔너리의 'value')는 따로
                string_variables.push(k);
                self.__string_variable_values[k] = [];
            }
            /* 바로 위의 Loop 를 수행하면,
             * 'self.__numeric_variables'는 다음과 같이 된다.
             * = ["sepal length", "sepal width", "petal length", "petal width"]
             *
             * 'self.__string_variable_values'는 다음과 같이 된다.
             * = { species: [] }
             *
             * */
        }
        ...
        //
        data.forEach(function (d) {
            for (var i in string_variables){
                var k = string_variables[i];
                var value = d[k];       // -> d[k] == d[species]
                if (self.__string_variable_values[k].indexOf(value) < 0)
                    self.__string_variable_values[k].push(value);
            }
        });
        /* 바로 위의 Loop 를 수행하면,
        * 'self.__string_variable_values'는 다음과 같다.
        * = { species: ["setosa", "versicolor", "virginica"] }*/
        //console.log(self.__string_variable_values);
        ...
        /* Control panel 생성 */
        var size_control = control.append("div")
            .attr("class", "scatter-matrix-size-control");
        var color_control = control.append("div")
            .attr("class", "scatter-matrix-color-control");
        var filter_control = control.append("div")
            .attr("class", "scatter-matrix-filter-control");
        var variable_control = control.append("div")
            .attr("class", "scatter-matrix-variable-control");
        var drill_control = control.append("div")
        ...
        function set_filter(variable) {
            ...
            if (variable){
                ...
                var filter_li = filter_control.append("p")
        ...
        color_control.append("p")
            .text("Select a variable to color");
        color_control.append("ul")
            .selectAll("li")
        ...
        var variable_li = variable_control.append("p")
        ...
        drill_li = drill_control.append("p")
        ...
        drill_li.append("input")
            .attr("type", "checkbox")
            .on('click', function (d, i) {
                var new_drill_variable = [];
                for (var j in drill_variables){
                    var v = drill_variables[j];
                    if (v !== d || this.checked) { new_drill_variable.push(v); }
                }
                if (this.checked){ new_drill_variable.push(d); }
                drill_variables = new_drill_variable;
                self.__draw(self.__cell_size, svg, color_variable, selected_colors, to_include, drill_variables);
            });
